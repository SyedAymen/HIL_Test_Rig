...
const filteredPoints = computed(() =>
  props.points.filter((p) => {
    ...
    if (statusFilter.value !== 'all') {
      const bucket = computeStatus(p, rig.bmsValues)
...
// Trace color is always status-based now (pass/fail/pending vs Carel bus).
function traceColor(p) {
  return STATUS_COLOR[computeStatus(p, rig.bmsValues)]
}
          ...
          >
            ...
            <div class="w-28 shrink-0 flex items-center justify-end gap-2">
              ...
              <span
                ...
                :style="{ backgroundColor: STATUS_COLOR[computeStatus(p, rig.bmsValues)] }"
                :title="`Status vs Carel bus: ${computeStatus(p, rig.bmsValues)}`"
              ></span>
